// //como en css h1{};; .parrafito{};;#pid{}
// const h1 = document.querySelector("h1")
// const p = document.querySelector("p")
// const parrafito = document.querySelector(".parrafito")
// const pid = document.getElementById("pid")//si lo llamo de esta forma no se pone el #, solo el nombre del id
// const input = document.querySelector("input")
// const allP = document.querySelectorAll("p")//devuelve una lista
// console.log(input.value)
// console.log({ 
//     h1,
//     p,
//     parrafito,
//     pid,
//     input
// })
// console.log(allP);
// h1.innerHTML="patito <br> FEO"; //nos permite modificar el html que este dentro de la etiqueta seleccionada
// h1.innerText="patito <br> FEO";//Convierte a texto, y proteje de amnezas
// console.log (h1.getAttribute("class")) //nos da el atributo seleccionado de la etiqueta seleccionada
// h1.setAttribute("class", "rojo")//cambia el atributo de la etiqueta selecc.
// h1.classList.add("azul") //agrega una clase
// h1.classList.remove("azul")//remueve una clase
// // h1.classList.toggle("verde")
// // h1.classList.contains("verde")
// input.value="231" //cambia el value del input
// //document.createElement("img") crea una img en el html
// const img = document.createElement("img")
// img.setAttribute("src","data:image/jpeg;base64,")
// console.log(img);
// pid.innerHTML="";
// pid.append(img);

const h1 = document.querySelector("h1")
const input1= document.querySelector("#calculo1")
const input2= document.getElementById("calculo2")
const btn =document.querySelector("#btnCalcular")
const resultP = document.getElementById("result")
const form = document.getElementById("form")

form.addEventListener("submit",sumarInputValues)

// btn.addEventListener("click", sumarInputValues) //("evento", codigoJS)escucha cada vez que suceda cierto evento y ejecuta un codigo de js(sin parentesis), que son sus dos parametros

// function sumarInputValues (event){
//     // console.log(event);
//     event.preventDefault(); //con este evento, logramos que el form no actualice la pagina(lo hace por defecto) 
//     const sumaInputs=(+input1.value + +input2.value) //los + los convierte de strings a numeros
//     resultP.innerText ="Resultado: " + sumaInputs;
// }

